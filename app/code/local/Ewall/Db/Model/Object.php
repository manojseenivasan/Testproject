 ...
 *
 */
class Ewall_Db_Model_Object extends Mage_Core_Model_Abstract {
    ...
    public function afterCommitCallback() {
		...
		if (!$this->getdata('_m_prevent_replication')) {
			Mage::helper('ewall_db')->replicate(array(
				'trackKeys' => true,
	...
	public function validateKeys() {
		$result = Mage::getModel('ewall_db/validation');
		$this->_validateKeys($result);
		Mage::dispatchEvent('ewall_database_validateKeys', array('object' => $this, 'result' => $result));
		if (count($result->getErrors())) {
			throw new Ewall_Db_Exception_Validation($result->getErrors());
		}
	}
	public function validate() {
		$result = Mage::getModel('ewall_db/validation');
		$this->_validate($result);
		Mage::dispatchEvent('ewall_database_validate', array('object' => $this, 'result' => $result));
		if (count($result->getErrors())) {
			throw new Ewall_Db_Exception_Validation($result->getErrors());
		}
	}
    ...
    public function assignDefaultValues() {
        ...
        Mage::dispatchEvent('ewall_database_assign_defaults', array('object' => $this));
        return $this;
    }
