 ...
 *
 */
class Ewall_Ajax_Model_Observer {
    /**
    ...
    public function ajaxifyPage($observer) {
        ...
        if ($allowedAjaxActions = Mage::helper('ewall_ajax')->getAllowedActions($actionName)) {
            ...
            if (($ajaxAction = $controller->getRequest()->getParam('m-ajax')) && in_array($ajaxAction, $allowedAjaxActions)) {
                Mage::helper('ewall_core')->updateRequestParameter('m-ajax', '', $ajaxAction);
                Mage::helper('ewall_core')->updateRequestParameter('no_cache', '', '1');
