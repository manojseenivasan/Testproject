 ...
 *
 */
class Ewall_FilterExpandCollapse_Model_Observer {
	/**
	...
	public function isConfigChanged($observer) {
		...
		Mage::helper('ewall_db')->checkIfPathsChanged($result, $configData, array(
			'ewall_filters/advanced/collapseable',
        ));
	}
    /**
    ...
    public function processUpdate($observer) {
		...
		switch ($object->getEntityName()) {
			case 'ewall_filters/filter2':
				if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_COLLAPSEABLE)) {
					$object->setCollapseable(Mage::helper('ewall_db')->getLatestConfig('ewall_filters/advanced/collapseable'));
				}
                break;
			case 'ewall_filters/filter2_store':
				if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_COLLAPSEABLE)) {
    ...
    public function addEditedData($observer) {
		...
		switch ($object->getEntityName()) {
			case 'ewall_filters/filter2':
			case 'ewall_filters/filter2_store':
				Mage::helper('ewall_db')->updateDefaultableField($object, 'collapseable', Ewall_Filters_Resource_Filter2::DM_COLLAPSEABLE, $fields, $useDefault);
                break;
		}
    }
    ...
    public function addFields($observer) {
        ...
        /* @var $t Ewall_FilterHelp_Helper_Data */ $t = Mage::helper(strtolower('Ewall_FilterExpandCollapse'));
...
//                    $wysiwygConfig = Mage::getSingleton('cms/wysiwyg_config')->getConfig(array('widget_filters' => $widgetFilters));
                    $field = $fieldset->addField('collapseable', 'select', array(
                        ...
                        'name' => 'collapseable',
                        'required' => false,
                        'options' => Mage::getSingleton('ewall_filterexpandcollapse/source_method')->getOptionArray(),
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_COLLAPSEABLE,
    ...
    public function renderCss($observer) {
        ...
        /* @var $helper Ewall_FilterExpandCollapse_Helper_Data */ $helper = Mage::helper(strtolower('Ewall_FilterExpandCollapse'));
    ...
    public function renderAction($observer) {
    	...
    	/* @var $result Varien_Object */ $result = $observer->getEvent()->getResult();
    	if ($helperBlock = $layout->getBlock('ewall_filter_expandcollapse')) {
