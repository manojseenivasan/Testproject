 ...
 *
 */
class Ewall_Core_Model_Html_Parser extends Varien_Object {
	protected $_token; 
	protected $_openedElements = array();
	
	protected function _construct() {
		$this->_token = $this->getReader()->read($this->hasStartsWith() ? $this->getStartsWith() : Ewall_Core_Model_Html_State::INITIAL_TEXT);
	}
	protected function _read($initialState, $expect = 0, $allowWhitespace = true) {
		...
		if ($expect && $this->_token['type'] != $expect) {
			throw new Exception(Mage::helper('ewall_core')->__('HTML parser error %s: %s expected%s', 
				Ewall_Core_Model_Html_Token::getPosition($this->_token), Ewall_Core_Model_Html_Token::getName($expect), 
	...
	public function parseContent($parentElement = null) {
		...
		while ($this->_token['type'] != Ewall_Core_Model_Html_Token::EOF && $this->_token['type'] != Ewall_Core_Model_Html_Token::TAG_END) {
			switch ($this->_token['type']) {
				case Ewall_Core_Model_Html_Token::TAG_START:
					...
					break;
				case Ewall_Core_Model_Html_Token::CDATA:
					$this->_processCDATA($parentElement, $result, $this->_token);
					$this->_read(Ewall_Core_Model_Html_State::INITIAL_TEXT);
					break;
				case Ewall_Core_Model_Html_Token::COMMENT:
					$this->_processComment($parentElement, $result, $this->_token);
					$this->_read(Ewall_Core_Model_Html_State::INITIAL_TEXT);
					break;
				case Ewall_Core_Model_Html_Token::TEXT:
					$this->_processText($parentElement, $result, $this->_token);
					$this->_read(Ewall_Core_Model_Html_State::INITIAL_TEXT);
					break;
				default: 
					throw new Exception(Mage::helper('ewall_core')->__('HTML parser error %s: unexpected %s%s', 
						Ewall_Core_Model_Html_Token::getPosition($this->_token), 
						Ewall_Core_Model_Html_Token::getName($this->_token['type']), 
						$this->getReader()->getSourceAt($this->_token)));
			}
		}
	...
	public function parseElement($parentContent = null) {
		...
		$this->_read(Ewall_Core_Model_Html_State::INITIAL, Ewall_Core_Model_Html_Token::NAME, false);
		$elementName = $this->_token['text'];
		array_push($this->_openedElements, $elementName);
		$void = Ewall_Core_Model_Html_Token::isVoid($elementName);
		$rawText = Ewall_Core_Model_Html_Token::isRawText($elementName);
		$this->_processElementName($parentContent, $result, $this->_token, $elementName, $void, $rawText);
		while ($this->_read(Ewall_Core_Model_Html_State::INITIAL) == Ewall_Core_Model_Html_Token::NAME || ($this->_token['type'] == Ewall_Core_Model_Html_Token::EQ)) {
			if ($this->_token['type'] != Ewall_Core_Model_Html_Token::EQ) {
				...
				$this->_read(Ewall_Core_Model_Html_State::INITIAL_VALUE, Ewall_Core_Model_Html_Token::VALUE, true);
				$attributeValue = $this->_token['text'];
				$this->_processAttributeValue($parentContent, $result, $this->_token, $attributeValue);
			}
		}
		switch ($this->_token['type']) {
			case Ewall_Core_Model_Html_Token::TAG_SELF_CLOSE: 
				$this->_processElementClose($parentContent, $result, $this->_token);
				array_pop($this->_openedElements);
				break;
			case Ewall_Core_Model_Html_Token::TAG_CLOSE: 
				$this->_processElementClose($parentContent, $result, $this->_token);
				if (!$void) {
					$this->_read($rawText ? $elementName : Ewall_Core_Model_Html_State::INITIAL_TEXT);
					...
					if ($this->_token['type'] != Ewall_Core_Model_Html_Token::TAG_END) {
						throw new Exception(Mage::helper('ewall_core')->__('HTML parser error %s: %s expected%s', 
							Ewall_Core_Model_Html_Token::getPosition($this->_token), 
							Ewall_Core_Model_Html_Token::getName(Ewall_Core_Model_Html_Token::TAG_END), 
							$this->getReader()->getSourceAt($this->_token)));
					}
					$this->_read(Ewall_Core_Model_Html_State::INITIAL, Ewall_Core_Model_Html_Token::NAME, false);
					array_pop($this->_openedElements);
					if ($this->_token['text'] != $elementName) {
						if (in_array($this->_token['text'], $this->_openedElements)) {
							...
							throw new Exception(Mage::helper('ewall_core')->__('HTML parser error %s: closing tag for %s expected%s', 
								Ewall_Core_Model_Html_Token::getPosition($this->_token), $elementName, 
								$this->getReader()->getSourceAt($this->_token)));
						}
					}
					else {
						$this->_read(Ewall_Core_Model_Html_State::INITIAL, Ewall_Core_Model_Html_Token::TAG_CLOSE, false);
						$this->_processElementEnd($parentContent, $result, $this->_token, $elementName);
					}
				}
				else {
				...
				break;
			default:
				throw new Exception(Mage::helper('ewall_core')->__('HTML parser error %s: %s or %s expected%s', 
					Ewall_Core_Model_Html_Token::getPosition($this->_token), 
					Ewall_Core_Model_Html_Token::getName(Ewall_Core_Model_Html_Token::TAG_SELF_CLOSE), 
					Ewall_Core_Model_Html_Token::getName(Ewall_Core_Model_Html_Token::TAG_CLOSE), 
					$this->getReader()->getSourceAt($this->_token)));
		}
		$this->_read(Ewall_Core_Model_Html_State::INITIAL_TEXT);
