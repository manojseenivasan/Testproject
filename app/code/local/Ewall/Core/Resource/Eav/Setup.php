 ...
 *
 */
class Ewall_Core_Resource_Eav_Setup extends Mage_Eav_Model_Entity_Setup {
    protected function _prepareValues($attr)
    {
        $data = parent::_prepareValues($attr);
        $data = array_merge($data, array(
            'is_key'		   			=> $this->_getValue($attr, 'is_key', 0),
            'is_global'		   			=> $this->_getValue($attr, 'is_global', Ewall_Core_Model_Attribute_Scope::_GLOBAL),
	...
	public function updateDefaultMaskFields($entity) {
		...
		$defaultMaskFields = $db->fetchCol("SELECT DISTINCT m.`default_mask_field` 
			FROM {$this->getTable('ewall_attributes')} AS m
