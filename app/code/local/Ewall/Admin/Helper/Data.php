...
/**
 * Generic helper functions for Ewall_Admin module. This class is a must for any module even if empty.
 * @author Ewall Team
 */
class Ewall_Admin_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function getStoreUrl($route, $params = array(), $query = array()) {
		if (!Mage::helper('ewall_admin')->isGlobal()) {
			$params['store'] = Mage::helper('ewall_admin')->getStore()->getId();
		}
		if (count($query)) {
			$params['_query'] = $query;
		}
    ...
    /**
     * @param int $id
     * @return Ewall_Filters_Model_Filter2_Value
     */
    ...
    public function getProductChooserHtml($beforeHtmlCallback = null) {
        ...
        $productsGrid = $layout->createBlock('ewall_admin/chooser_product', '', array(
            'id' => $uniqId,
        ...
        if (!$request->getParam('products_grid')) {
            ...
            $html = $layout->createBlock('adminhtml/template')
                    ->setTemplate('ewall/admin/chooser/product.phtml')
    ...
    public function getCmsBlockChooserHtml($beforeHtmlCallback = null) {
        ...
        $cmsBlocksGrid = $layout->createBlock('ewall_admin/chooser_cmsblock', '', array(
            'id' => $uniqId,
            'use_massaction' => $massAction,
        ));
        ...
        $html = $layout->createBlock('adminhtml/template')
                ->setTemplate('ewall/admin/chooser/cmsblock.phtml')
