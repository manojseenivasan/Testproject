 ...
 *
 */
class Ewall_FilterSuperSlider_Model_Observer extends Mage_Core_Helper_Abstract {
    /**
            ...
            case 'ewall_filters/filter2_store':
                if ($form->getId() == 'mf_general') {
                    ...
                    $fieldset->addField('slider_number_format', 'text', array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Number Format'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('0 will be replaced by actual number; all other characters will be displayed as entered.'),
                        'name' => 'slider_number_format',
                        'required' => true,
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT,
                        'default_label' => $admin->isGlobal()
                            ? Mage::helper('ewall_filtersuperslider')->__('Use Price Format')
                            : Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    ))->setRenderer($fieldRenderer);
                    $fieldset->addField('slider_decimal_digits', 'text', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Digits After the Decimal Point'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('Leave empty or 0 to round to whole numbers.'),
                        'name' => 'slider_decimal_digits',
                        'required' => true,
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    $fieldset->addField('slider_threshold', 'text', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Use Second Number Format If Value Is Greater Than'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('Leave empty or 0 to use basic Number Format above for all values.'),
                        'name' => 'slider_threshold',
                        'required' => false,
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_THRESHOLD,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    $fieldset->addField('slider_number_format2', 'text', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Second Number Format'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('Only applicable for values greater than parameter specified above. 0 will be replaced by actual number divided by the parameter specified above; all other characters will be displayed as entered.'),
                        'name' => 'slider_number_format2',
                        'required' => true,
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT2,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    $fieldset->addField('slider_decimal_digits2', 'text', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Digits After the Decimal Point (for Second Format)'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('Leave empty or 0 to round to whole numbers.'),
                        'name' => 'slider_decimal_digits2',
                        'required' => true,
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS2,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    /** @noinspection PhpUndefinedMethodInspection */
                    $fieldset->addField('slider_manual_entry', 'select', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Allow Customers to Enter Range Manually'),
                        'name' => 'slider_manual_entry',
                        'required' => true,
                        'options' => Mage::getSingleton('ewall_core/source_yesno')->getOptionArray(),
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_MANUAL_ENTRY,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    /** @noinspection PhpUndefinedMethodInspection */
                    $fieldset->addField('slider_use_existing_values', 'select', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Slide Only Through Existing Values'),
                        'name' => 'slider_use_existing_values',
                        'required' => true,
                        'options' => Mage::getSingleton('ewall_core/source_yesno')->getOptionArray(),
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_SLIDER_USE_EXISTING_VALUES,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    /** @noinspection PhpUndefinedMethodInspection */
                    $fieldset->addField('thousand_separator', 'select', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Show Thousand Separator'),
                        'name' => 'thousand_separator',
                        'required' => true,
                        'options' => Mage::getSingleton('ewall_core/source_yesno')->getOptionArray(),
                    ), $admin->isGlobal() ? array() : array(
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_THOUSAND_SEPARATOR,
                        'default_label' => Mage::helper('ewall_filtersuperslider')->__('Same For All Stores'),
                    )))->setRenderer($fieldRenderer);
                    ...
                    $fieldset->addField('range_step', 'text', array_merge(array(
                        'label' => Mage::helper('ewall_filtersuperslider')->__('Range Step'),
                        'note' => Mage::helper('ewall_filtersuperslider')->__('When showing filter options, system will divide the whole range of values to segments of specified size. Leave empty or 0 for automatic step calculation.'),
                        'name' => 'range_step',
                        'required' => true,
                        'default_bit' => Ewall_Filters_Resource_Filter2::DM_RANGE_STEP,
                        'default_label' => $admin->isGlobal()
	...
	public function processUpdate($observer) {
		...
		switch ($object->getEntityName()) {
			case 'ewall_filters/filter2':
				if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT)) {
					$object->setSliderNumberFormat($this->_getDefaultSliderNumberFormat());
				}
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_RANGE_STEP)) {
			...
			case 'ewall_filters/filter2_store':
                ...
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_MANUAL_ENTRY)) {
                    $object->setSliderManualEntry($values['slider_manual_entry']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT2)) {
                    $object->setSliderNumberFormat2($values['slider_number_format2']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_THRESHOLD)) {
                    $object->setSliderThreshold($values['slider_threshold']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_USE_EXISTING_VALUES)) {
                    $object->setSliderUseExistingValues($values['slider_use_existing_values']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS)) {
                    $object->setSliderDecimalDigits($values['slider_decimal_digits']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS2)) {
                    $object->setSliderDecimalDigits2($values['slider_decimal_digits2']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_RANGE_STEP)) {
                    $object->setRangeStep($values['range_step']);
                }
                if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_THOUSAND_SEPARATOR)) {
    ...
    public function addEditedData($observer) {
        ...
        switch ($object->getEntityName()) {
            case 'ewall_filters/filter2':
            case 'ewall_filters/filter2_store':
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_number_format', Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_manual_entry', Ewall_Filters_Resource_Filter2::DM_SLIDER_MANUAL_ENTRY, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_number_format2', Ewall_Filters_Resource_Filter2::DM_SLIDER_NUMBER_FORMAT2, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_threshold', Ewall_Filters_Resource_Filter2::DM_SLIDER_THRESHOLD, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_use_existing_values', Ewall_Filters_Resource_Filter2::DM_SLIDER_USE_EXISTING_VALUES, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_decimal_digits', Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'slider_decimal_digits2', Ewall_Filters_Resource_Filter2::DM_SLIDER_DECIMAL_DIGITS2, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'range_step', Ewall_Filters_Resource_Filter2::DM_RANGE_STEP, $fields, $useDefault);
                Mage::helper('ewall_db')->updateDefaultableField($object, 'thousand_separator', Ewall_Filters_Resource_Filter2::DM_THOUSAND_SEPARATOR, $fields, $useDefault);
                break;
        }
    }
    ...
    public function validate($observer) {
        ...
        switch ($object->getEntityName()) {
            case 'ewall_filters/filter2':
            case 'ewall_filters/filter2_store':
                $t = Mage::helper('ewall_filtersuperslider');
    ...
    protected function _getDefaultSliderNumberFormat() {
        ///* @var $helper Ewall_Filters_Helper_Data */ $helper = Mage::helper(strtolower('Ewall_Filters'));
        //return $helper->getJsPriceFormat();
        return '$0';
    }
    protected function _getDefaultRangeStep($object) {
        if ($object->getCode() == 'price') {
            return Mage::helper('ewall_db')->getLatestConfig('catalog/layered_navigation/price_range_calculation') == 'auto' ? 0 :
                    Mage::helper('ewall_db')->getLatestConfig('catalog/layered_navigation/price_range_step');
        }
        else {
            return 0;
        }
    ...
    public function renderPriceRange($observer) {
        ...
        /* @var $model Ewall_Filters_Model_Filter_Decimal */ $model = $observer->getEvent()->getModel();
        /* @var $result Varien_Object */ $result = $observer->getEvent()->getResult();

        /* @var $helper Ewall_FilterSuperSlider_Helper_Data */
        $helper = Mage::helper(strtolower('Ewall_FilterSuperSlider'));
    ...
    public function addRangeStepToCollection($observer) {
        /* @var $collection Ewall_Filters_Resource_Filter2_Store_Collection */ $collection = $observer->getEvent()->getCollection();
    ...
    public function renderMenuContainerCss($observer) {
        ...
        if ($filter->getFilterOptions()->getDisplay() == 'slider') {
            if (Mage::getStoreConfig('ewall_filters/positioning_menu/inline_slider') == 'inline') {
                echo ' m-inline';
            }
        }
    }
