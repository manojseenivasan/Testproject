 ...
 *
 */
class Ewall_FilterSeoLinks_Model_Url extends Mage_Core_Model_Url {
    ...
    public function encodeUrl($routePath, $result) {
		...
		if ($routePath == 'catalog/category/view' || $routePath == 'ewall_filterajax/category/view') {
		    $this->_encodeUrl($routePath, $result);
		}
		elseif ($routePath == 'cms/index/index' || $routePath == 'ewall_filterajax/index/index') {
		    $this->_encodeUrl($routePath, $result, array(
		        'handleCategorySuffix' => false,
		    ));
		}
        elseif ($routePath == 'cms/page/view' || $routePath == 'ewall_filterajax/page/view') {
            $this->_encodeUrl($routePath, $result, array(
		        'handleCategorySuffix' => false,
		    ));
        }
    ...
    protected function _encodeUrl($routePath, &$result, $options = array()) {
        ...
        /* @var $resource Ewall_FilterSeoLinks_Resource_Rewrite */ $resource = Mage::getResourceSingleton('ewall_filterseolinks/rewrite');
        $conditionalWord = $core->getStoreConfig('ewall_filters/seo/conditional_word');
        $categorySuffix = $options['handleCategorySuffix'] ? Mage::helper('ewall_filterseolinks')->getCategoryUrlSuffix() : '';
        $showAllSuffix = $core->getStoreConfig('ewall_filters/seo/show_all_suffix');
        if ($showAllSuffix) $showAllSeoSuffix = $showAllSuffix;
        $vars = Mage::helper('ewall_filterseolinks')->getUrlVars();
            ...
            foreach ($query as $key => $value) {
                if ($showAllSuffix && strrpos($key, $showAllSuffix) === strlen($key) - strlen($showAllSuffix)) {
                    ...
                    if ($key == 'cat') {
                        ...
                        if ($seoValue !== '') {
                            $seoQuery .= '/' . Mage::helper('ewall_filterseolinks')->getCategoryName() . '/' . $seoValue;
                        }
                    }
                    ...
                    elseif ($filter = $resource->isFilterName($key)) {
                        $seoName = Mage::getStoreConfigFlag('ewall_filters/seo/use_label') ? $core->labelToUrl($filter->getLowerCaseName()) : $core->hyphenCased($key);
                        if (strpos($value, '__0__') === false) {
                            $valueLabel = array();
                            foreach (explode('_', $value) as $valueFragment) {
    ...
    public function encodeValue($code, $value) {
        $resource = Mage::getResourceSingleton('ewall_filterseolinks/rewrite');
        /* @var $resource Ewall_FilterSeoLinks_Resource_Rewrite */
        if ($code == 'cat') {
