 ...
 *
 */
class Ewall_FilterSeoLinks_Model_Observer extends Mage_Core_Helper_Abstract {
    ...
    protected function _addAppliedFiltersToCategoryTitle($head, $appliedFilters, $page) {
        if (count($appliedFilters) || $page && $page > 1) {
            ...
            foreach ($appliedFilters as /* @var $item Ewall_Filters_Model_Item */$item) {
            ...
            if (file_exists(BP . '/app/etc/m_page_titles.xml')) {
                ...
                $xml = simplexml_load_string(file_get_contents(BP . '/app/code/local/Ewall/FilterSeoLinks/etc/m_page_titles.xml'));
            }
	...
	protected function _processCase($rule, &$globalVars) {
        ...
        if (!empty($rule['category_id'])) {
            /* @var $filters Ewall_Filters_Helper_Data */
            $filters = Mage::helper(strtolower('Ewall_Filters'));
            $layer = $filters->getLayer();
    ...
    protected function _noindex($layerModel) {
        if (($head = Mage::getSingleton('core/layout')->getBlock('head'))) {
            ...
            foreach (explode(',', Mage::getStoreConfig('ewall_filters/seo/no_index')) as $noindexProcessorName) {
                if (!$noindexProcessorName) {
                    continue;
                }

                $noindexProcessor = Mage::getModel((string)Mage::getConfig()->getNode('ewall_filterseolinks/noindex')->$noindexProcessorName->model);
