 ...
 *
 */
class Ewall_FilterSeoLinks_Resource_Rewrite extends Mage_Core_Model_Mysql4_Url_Rewrite {
	public function isFilterName($name) {
		Ewall_Core_Profiler::start('mln'.'::'. __CLASS__ . '::' . __METHOD__ . '::' . $name);
		...
		Ewall_Core_Profiler::stop('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . $name);
		return $result;
	}
    public function getFilterName($candidates) {
		...
		foreach ($collection as $item) {
		    if (Mage::getStoreConfigFlag('ewall_filters/seo/use_label')) {
	...
	public function getCategoryValue($model, $urlKey) {
		Ewall_Core_Profiler::start('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . 'select');
		...
		Ewall_Core_Profiler::stop('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . 'select');
	...
	public function getCategoryLabel($categoryId) {
        ...
        $currentCategoryId = Mage::helper('ewall_filters')->getLayer()->getCurrentCategory()->getId();
        $path = explode('/', $path);
