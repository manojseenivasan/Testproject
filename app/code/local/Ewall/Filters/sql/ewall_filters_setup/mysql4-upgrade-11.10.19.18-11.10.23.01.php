...
$installer->run("
	...
	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_ewall_db/edit_session` FOREIGN KEY (`edit_session_id`) 
	  REFERENCES `{$installer->getTable('ewall_db/edit_session')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_ewall_filters/filter2` FOREIGN KEY (`filter_id`) 
	  REFERENCES `{$installer->getTable('ewall_filters/filter2')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
/* BASED ON SNIPPET: Resources/Table creation/alteration script */
$table = 'ewall_filternew_value_store';
$installer->run("
	DROP TABLE IF EXISTS `{$this->getTable($table)}`;
	CREATE TABLE `{$this->getTable($table)}` (
	  `id` bigint NOT NULL AUTO_INCREMENT,
	...
	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_ewall_filters/filter2_store` FOREIGN KEY (`filter_id`) 
	  REFERENCES `{$installer->getTable('ewall_filters/filter2_store')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
