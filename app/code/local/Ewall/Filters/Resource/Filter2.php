...
/**
 * This resource model handles DB operations with a single model of type Ewall_Filters_Model_Filter2. All 
 * database specific code for Ewall_Filters_Model_Filter2 should go here.
 * @author Ewall Team
 */
class Ewall_Filters_Resource_Filter2 extends Ewall_Db_Resource_Object {
	...
	protected function _construct() {
        $this->_init(strtolower('Ewall_Filters/Filter2'), 'id');
	 ...
	 * @param Ewall_Db_Model_Replication_Target $target
	 */
	...
	/**
	 * Enter description here ...
	 * @param Ewall_Db_Model_Object $object
	 * @param array $values
	 * @param array $options
	 */
	protected function _processReplicationUpdate($object, $values, $options) {
		...
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_IS_ENABLED)) {
			$object->setIsEnabled($values['is_enabled']);
		}
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_DISPLAY)) {
			$object->setDisplay(Mage::helper('ewall_db')->getLatestConfig('ewall_filters/display/'.$object->getType()));
		}
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_NAME)) {
			$object->setName($values['name']);
		}
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_IS_ENABLED_IN_SEARCH)) {
			$object->setIsEnabledInSearch($values['is_enabled_in_search']);
		}
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_POSITION)) {
			$object->setPosition($values['position']);
		}
        if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2::DM_SORT_METHOD)) {
            $object->setSortMethod(Mage::helper('ewall_db')->getLatestConfig('ewall_filters/display/sort_method'));
        }
    }
	...
	protected function _addEditedData($object, $fields, $useDefault) {
		Mage::helper('ewall_db')->updateDefaultableField($object, 'is_enabled', Ewall_Filters_Resource_Filter2::DM_IS_ENABLED, $fields, $useDefault);
		Mage::helper('ewall_db')->updateDefaultableField($object, 'display', Ewall_Filters_Resource_Filter2::DM_DISPLAY, $fields, $useDefault);
		Mage::helper('ewall_db')->updateDefaultableField($object, 'name', Ewall_Filters_Resource_Filter2::DM_NAME, $fields, $useDefault);
		Mage::helper('ewall_db')->updateDefaultableField($object, 'is_enabled_in_search', Ewall_Filters_Resource_Filter2::DM_IS_ENABLED_IN_SEARCH, $fields, $useDefault);
		Mage::helper('ewall_db')->updateDefaultableField($object, 'position', Ewall_Filters_Resource_Filter2::DM_POSITION, $fields, $useDefault);
        Mage::helper('ewall_db')->updateDefaultableField($object, 'sort_method', Ewall_Filters_Resource_Filter2::DM_SORT_METHOD, $fields, $useDefault);
        Mage::helper('ewall_db')->updateDefaultableField($object, 'operation', Ewall_Filters_Resource_Filter2::DM_OPERATION, $fields, $useDefault);
        Mage::helper('ewall_db')->updateDefaultableField($object, 'is_reverse', Ewall_Filters_Resource_Filter2::DM_IS_REVERSE, $fields, $useDefault);
    }
    protected function _afterSave(Mage_Core_Model_Abstract $object) {
        if ($edit = $object->getValueData()) {
            foreach ($edit['saved'] as $id => $editId) {
                if ($id > 0) {
                    $editModel = Mage::helper('ewall_admin')->loadModel('ewall_filters/filter2_value', $editId);
                    $data = $editModel->getData();
                    unset($data['id']);
                    unset($data['edit_status']);
                    unset($data['edit_session_id']);
                    $model = Mage::helper('ewall_admin')->loadModel('ewall_filters/filter2_value', $id)->addData($data);
