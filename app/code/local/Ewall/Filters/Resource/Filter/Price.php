 ...
 * Ewall_Filters_Model_Filter_Price::_getResource().
 */
class Ewall_Filters_Resource_Filter_Price extends Mage_Catalog_Model_Resource_Eav_Mysql4_Layer_Filter_Price {
     ...
     * @param Ewall_Filters_Model_Filter_Price $model
     * @return Ewall_Filters_Resource_Filter_Price
     */
    public function countOnCollection($collection, $model) {
        if (Mage::helper('ewall_core')->isMageVersionEqualOrGreater('1.7')) {
            ...
            Mage::helper('ewall_filters')->resetProductCollectionWhereClause($select);
...
//        $this->getCount($filter, 1);
    ...
    protected function _getConfigurablePriceFix() {
        if (!Mage::getStoreConfigFlag('ewall_filters/general/adjust_configurable_price')) {
            return '';
        }
        ...
        $values = array();
        foreach (Mage::helper('ewall_filters')->getFilterOptionsCollection() as $filter) {
            if ($filter->getType() == 'attribute' && ($param = $request->getParam($filter->getCode()))) {
                $values = array_merge($values, Mage::helper('ewall_core')->sanitizeNumber($param, array('_')));
            }
        }
        if (count($values) > 0) {
