...
/**
 * This resource model handles DB operations with a single model of type Ewall_Filters_Model_Filter2_Value_Store. All 
 * database specific code for Ewall_Filters_Model_Filter2_Value_Store should go here.
 * @author Ewall Team
 */
class Ewall_Filters_Resource_Filter2_Value_Store extends Ewall_Filters_Resource_Filter2_Value {
    /**
	...
	protected function _construct() {
        $this->_init(strtolower('Ewall_Filters/Filter2_Value_Store'), 'id');
        $this->_isPkAutoIncrement = false;
    }   
	protected function _getReplicationSources() {
		return array('ewall_filters/filter2_value', 'core/store', 'eav/attribute', 'ewall_filters/filter2_store');
	}
    /**
	...
	protected function _prepareReplicationUpdateSelects($target, $options) {
		...
		$select
			...
			->joinInner(array('parent' => Mage::getSingleton('core/resource')->getTableName('ewall_filters/filter2_store')), 
		...
		if ($options['trackKeys']) {
			...
			if (($keys = $options['targets']['ewall_filters/filter2_store']->getSavedKeys()) && count($keys)) {
	...
	protected function _processReplicationUpdate($object, $values, $options) {
		...
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2_Value::DM_NAME)) {
			$object->setName($values['name']);
		}
		if (!Mage::helper('ewall_db')->hasOverriddenValue($object, $values, Ewall_Filters_Resource_Filter2_Value::DM_POSITION)) {
	...
	protected function _prepareReplicationInsertSelects($target, $options) {
		...
		$select
			...
			->joinInner(array('parent_global' => Mage::getSingleton('core/resource')->getTableName('ewall_filters/filter2')), 
