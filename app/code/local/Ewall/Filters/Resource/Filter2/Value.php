...
/**
 * This resource model handles DB operations with a single model of type Ewall_Filters_Model_Filter2_Value. All 
 * database specific code for Ewall_Filters_Model_Filter2_Value should go here.
 * @author Ewall Team
 */
class Ewall_Filters_Resource_Filter2_Value extends Ewall_Db_Resource_Object {
	...
	protected function _construct() {
        $this->_init(strtolower('Ewall_Filters/Filter2_Value'), 'id');
        $this->_isPkAutoIncrement = false;
    }   
	protected function _getReplicationSources() {
		return array('eav/attribute', 'ewall_filters/filter2');
	}
	/**
	...
	protected function _prepareReplicationUpdateSelects($target, $options) {
		...
		$select
			...
			->joinInner(array('parent' => Mage::getSingleton('core/resource')->getTableName('ewall_filters/filter2')), 
		...
		if ($options['trackKeys']) {
			...
			if (($keys = $options['targets']['ewall_filters/filter2']->getSavedKeys()) && count($keys)) {
