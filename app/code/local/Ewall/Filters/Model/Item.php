 ...
 * @method Ewall_Filters_Model_Item setMSelected(bool $value)
 * @author Ewall Team
 * Injected instead of standard catalog/layer_filter_item in Ewall_Filters_Model_Filter_Attribute::_createItemEx()
 * method.
 */
class Ewall_Filters_Model_Item extends Mage_Catalog_Model_Layer_Filter_Item {
    /**
	...
	public function getSeoValue() {
        ...
        if (Mage::app()->getRequest()->getParam('m-seo-enabled', true) &&
            ((string)Mage::getConfig()->getNode('modules/Ewall_FilterSeoLinks/active')) == 'true' &&
            $this->getFilter()->getMode() != 'search'
        )
        {
            $url = Mage::getModel('ewall_filterseolinks/url');
            /* @var $url Ewall_FilterSeoLinks_Model_Url */
