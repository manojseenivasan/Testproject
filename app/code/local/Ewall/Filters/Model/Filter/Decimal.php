 ...
 * @author Ewall Team
 * Injected instead of standard catalog/layer_filter_attribute in Ewall_Filters_Block_Filter_Price constructor.
 */
class Ewall_Filters_Model_Filter_Decimal
    extends Mage_Catalog_Model_Layer_Filter_Decimal
    implements Ewall_Filters_Interface_Filter
{
    /**
    ...
    #region Ewall_Filters_Interface_Filter methods
    /**
    ...
    #endregion
    #region common part for all ewall_filters/filter_* models
     ...
     * @param array $data
     * @return Ewall_Filters_Model_Item
     */
    protected function _createItemEx($data)
    {
        return Mage::getModel('ewall_filters/item')
                ->setData($data)
                ->setFilter($this);
    }
     ...
     * @return Ewall_Filters_Model_Filter_Attribute
     * @see Mage_Catalog_Model_Layer_Filter_Abstract::_initItems()
     */
    protected function _initItems()
    {
        /* @var $ext Ewall_Filters_Helper_Extended */
        $ext = Mage::helper(strtolower('Ewall_Filters/Extended'));
    ...
    public function getRange()
    {
        ...
        if (!$range) {
            if (Mage::helper('ewall_db')->hasOverriddenValueEx($this->getFilterOptions(), 24)) {
                $range = (float)$this->getFilterOptions()->getRangeStep();
            } elseif (Mage::helper('ewall_db')->hasOverriddenValueEx(
    ...
    public function getMSelectedValues() {
        $values = Mage::helper('ewall_core')->sanitizeRequestNumberParam($this->_requestVar, array(
