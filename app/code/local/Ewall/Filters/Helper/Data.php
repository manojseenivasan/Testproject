...
/**
 * Generic helper functions for Ewall_Filters module. This class is a must for any module even if empty.
 * @author Ewall Team
 */
class Ewall_Filters_Helper_Data extends Mage_Core_Helper_Abstract {
	/**
	...
	public function getFilterName($block, $model) {
		if ($model instanceof Ewall_Filters_Model_Filter_Category) {
            $result = 'category';
		}
		else {
	...
	public function getFilterOptionsCollection($allCategories = false) {
	    ...
	    if ($request->getModuleName() == 'catalogsearch' && $request->getControllerName() == 'result' && $request->getActionName() == 'index' ||
	        $request->getModuleName() == 'ewall_filterajax' && $request->getControllerName() == 'search' && $request->getActionName() == 'index')
	    {
            if (!$this->_filterSearchOptionsCollection) {
                $this->_filterSearchOptionsCollection = Mage::getResourceModel('ewall_filters/filter2_store_collection')
                        ->addColumnToSelect('*')
		...
		if ($allCategories) {
			if (!$this->_filterAllOptionsCollection) {
				$this->_filterAllOptionsCollection = Mage::getResourceModel('ewall_filters/filter2_store_collection')
		        	->addColumnToSelect('*')
		        	->addStoreFilter(Mage::app()->getStore())
		        	->setOrder('position', 'ASC');
			}
			...
			if (!$this->_filterOptionsCollection) {
				Ewall_Core_Profiler::start('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . '$productCollection->getSetIds()');
				$setIds = Mage::getSingleton('catalog/layer')->getProductCollection()->getSetIds();
				Ewall_Core_Profiler::stop('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . '$productCollection->getSetIds()');
				$this->_filterOptionsCollection = Mage::getResourceModel('ewall_filters/filter2_store_collection')
		        	->addFieldToSelect('*')
	...
	public function markLayeredNavigationUrl($url, $routePath, $routeParams) {
        ...
        if ($path == 'catalog/category/view') {
            if (Mage::getStoreConfigFlag('ewall_filters/session/save_applied_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
        elseif ($path == 'catalogsearch/result/index') {
            if (Mage::getStoreConfigFlag('ewall_filters/session/save_applied_search_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
        else {
            if (Mage::getStoreConfigFlag('ewall_filters/session/save_applied_cms_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
		return $url;
    ...
    public function getMode() {
        ...
        elseif (in_array(Mage::helper('ewall_core')->getRoutePath(), array('catalogsearch/result/index', 'ewall_filterajax/search/index'))) {
            return 'search';
        }
        else {
            return 'category';
    ...
    public function getNoGroups($filterBlocks) {
        ...
        foreach ($filterBlocks as /* @var $filterBlock Ewall_Filters_Block_Filter */ $filterBlock) {
    ...
    /**
     * @param string $field
     * @param Ewall_Filters_Model_Filter2_Store $options
     * @return string
     */
